...
## Project
For this project, I developed a PCB with 7 flickering LEDs. My goal was to create a PCB depicting a haunted house backlit by flickering lights inside the house's windows, to serve as a Halloween decoration.

## Design Feature
I routed ground between two different pathways (one for D4, and D5, another for D1, D2, and D3) by using D6 to split it across two different branches of the PCB. This ensured that each LED's other terminal could connect to its respective resistor without a short. This was made more difficult by the numerous holes in the board (which form the "windows" in the haunted house). Initially, it was difficult to identify where the branched pathway should go because of the unequal distributions of space for LEDs (such that they could still be bent to light up the windows) across the board and the narrow board spaces available for wire. In the end, I feel that the solution I found was elegant since the wiring was fairly evenly distributed throughout (improving the ease of debugging down the line).
